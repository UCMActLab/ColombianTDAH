...
public class MisionLevelManager : MonoBehaviour
{
    ...
    HashSet<int> _askedSleepTimes = new HashSet<int>(); //para no repetir la pregunta de sueño en horas que ya se han hecho
    ...
    public void Answered(bool yes)
    {
        ...
        {
            if (!yes) _sleptHours -= _hourPerSleep; //si no duerme se restan horas de sueño
            _sleeping = yes;
            Sleep(yes);
            _shouldSleep = false;
